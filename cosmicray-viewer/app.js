class CosmicRayViewer {
    ...
    filterIds(searchTerm) {
        ...
        this.updateMapWithUsers(filtered);
    }
    
    updateMapWithUsers(filteredIds = null) {
        // 既存のマーカーをクリア
        ...
        const idsToShow = filteredIds || this.allIds;
        
        // GPS座標があるIDにマーカーを追加
        idsToShow.forEach(item => {
            let lat, lng, comment, isUserLocation = false;
            
            // まずuser情報から取得を試行（新しいAPI）
            if (item.user && item.user.gps_latitude && item.user.gps_longitude) {
                lat = parseFloat(item.user.gps_latitude);
                lng = parseFloat(item.user.gps_longitude);
                comment = item.user.comment || 'No comment';
                isUserLocation = true;
            }
            // フォールバック: config情報から取得（既存のAPI）
            else if (item.config && item.config.gps_latitude && item.config.gps_longitude) {
                lat = parseFloat(item.config.gps_latitude);
                lng = parseFloat(item.config.gps_longitude);
                comment = item.config.comment || 'No comment';
                ...
                const marker = L.marker([lat, lng])
                    ...
                    .bindPopup(`
                        <div style="color: #333;">
                            ...
                            <small style="color: #666;">${isUserLocation ? 'User GPS Location' : 'Data GPS Location'}</small><br>
    ...
    updateMap(ids) {
        // 後方互換性のため残す（updateMapWithUsersを呼び出し）
        this.updateMapWithUsers(ids);
    }
