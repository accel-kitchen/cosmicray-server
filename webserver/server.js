    ...
    try {
        ...
        const ids = fs.readdirSync(dataDir, { withFileTypes: true })
            ...
            .map(dirent => {
                ...
                // ユーザー情報を追加
                const userInfo = usersMap[dirent.name] || null;
...
app.put('/admin/users/:id', authenticateToken, requireAdmin, async (req, res) => {
    const { id } = req.params;
    const { comment, gps_latitude, gps_longitude, password } = req.body;
    ...
    try {
        ...
        if (gps_latitude !== undefined) user.gps_latitude = gps_latitude;
        if (gps_longitude !== undefined) user.gps_longitude = gps_longitude;
        ...
        if (saveUsers(userData)) {
            res.json({
                ...
                user: {
                    ...
                    gps_latitude: newUser.gps_latitude,
                    gps_longitude: newUser.gps_longitude,
